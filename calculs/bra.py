def bra(Class):
    ...
    if (Class.type_bar == "degressif" or Class.type_bar == "  "):
        ...
        if (Class.F0 == "année"):
            Class.BRA = "La barrière de remboursement anticipé automatique est dégressive au fil du temps. Elle est fixée à <BAC> du <NDR>  en fin d'<F0> <1PR>, puis décroît de " + str(str(deg).replace(".",",")) +"% chaque <F0>, pour atteindre <ABDAC>% du <NDR> à la fin du <F0> <ADPR>."

        Class.BVC = Class.DBAC + "% du <NDR> <SJR7>"